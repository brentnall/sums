...
filter_mortality_data <- function(mortality_data, cancer) {
    ##cancer type
    mortality_data$main_timepoint_RoB[mortality_data$main_timepoint_RoB=="Yes"] <-"yes" 
...
final_table <- function(mortality_data, surrogate_data){
    mortality_data <- mortality_data %>%
        mutate(yim2=as.numeric(yim2)) %>%
    select(trial_acronym, screening_test, outcome_description, numerator_screening, numerator_comparator, denominator_screening, denominator_comparator, yi, vi, sei, ci.lb, ci.ub, yim2)
...
# - weighted correlation
# - returns line for results table
## as above but uses rate difference for mortality, not relative risk
analysis2 <- function(finaldata_primary, cancer, surrogate, analysis, bootfunction){

  finaldata_primary <- finaldata_primary %>%
    filter(!is.na(yi_s))
    
  # Regression (inverse variance weights)
  m_reg <- lm(formula = yim2 ~ yi_s, data = finaldata_primary, weights = weight_var)
  m_reg_sum <- summary(m_reg)
  
  cis <- bootfunction(finaldata_primary)
  
  # correlation
  correlation <- weights::wtd.cor(finaldata_primary$yim2, finaldata_primary$yi_s, weight=finaldata_primary$weight_var)
